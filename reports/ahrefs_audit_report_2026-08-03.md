# Ahrefs Site-Audit-Report – naturheilpraxis-straehuber.de
_Stand: 2026-08-03_

**Health Score:** 86/100 – +0 ggue. letztem Lauf (86)
**Gepruefte Seiten:** 391 (55 Fehler, 103 Warnungen, 163 Hinweise)

## Probleme mit betroffenen Seiten (nur aktive Probleme)

| Wichtigkeit | Problem | Kategorie | Betroffene Seiten | Aenderung ggue. letztem Lauf |
|---|---|---|---|---|
| Error | Orphan page (has no incoming internal links) | Links | 33 | unveraendert |
| Error | Image file size too large | Images | 19 | unveraendert |
| Error | Timed out | Internal pages | 3 | unveraendert |
| Warning | Title too long | Content | 57 | unveraendert |
| Warning | H1 tag missing or empty | Content | 33 | unveraendert |
| Warning | Meta description too short | Content | 13 | unveraendert |
| Warning | Meta description too long | Content | 4 | unveraendert |
| Warning | Missing alt text | Images | 2 | unveraendert |
| Warning | Meta description tag missing or empty | Content | 1 | unveraendert |
| Warning | Noindex page | Indexability | 1 | unveraendert |
| Notice | Pages to submit to IndexNow | Other | 105 | unveraendert |
| Notice | Title tag changed | Content | 91 | unveraendert |
| Notice | Structured data has Google rich results validation error | Other | 71 | unveraendert |
| Notice | Meta description changed | Content | 8 | unveraendert |
| Notice | Multiple H1 tags | Content | 4 | unveraendert |
| Notice | Structured data has schema.org validation error | Other | 3 | unveraendert |
| Notice | H1 tag changed | Content | 3 | unveraendert |
| Notice | Page and SERP titles do not match | Content | 3 | unveraendert |
| Notice | Indexable page became non-indexable | Indexability | 3 | unveraendert |
| Notice | Meta description too short | Content | 1 | unveraendert |
| Notice | SERP title changed | Content | 1 | unveraendert |
| Notice | Noindex follow page | Indexability | 1 | unveraendert |

## Automatische Bild-Optimierung (diese Woche)

- **18 Bilder** automatisch komprimiert und ersetzt (WebP), **24.37 MB gespart**
- 1 SVG(s) übersprungen (Vektorformat, keine WebP-Konvertierung)

## Automatische Alt-Text-Ergänzung (diese Woche)

- **0 Alt-Texte** automatisch per Gemini generiert und gesetzt (2 geprüfte Seiten)